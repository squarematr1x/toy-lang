...
$(APP_TEST_DIR)/$(TEST_TARGET): $(TEST_OBJECTS)
	@mkdir -p $(@D)
	$(CXX) $(CXX_FLAGS) -o $(APP_TEST_DIR)/$(TARGET) $^ $(GTEST_SRC) $(GTEST_INCLUDE) $(LD_TEST_FLAGS)

-include $(DEPENDENCIES)

.PHONY: all build clean build_test test debug release info

build:
	@mkdir -p $(APP_DIR)
	@mkdir -p $(OBJ_DIR)

build_test: build $(APP_TEST_DIR)/$(TEST_TARGET)
